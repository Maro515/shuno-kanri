...
(function () {
  ...
  // --------------------------------------------------------------- storage --
  /* 共有端末で全員が使うため、進捗・履歴は一切保存しない（メモリにも残さない） */
  function loadProgress() { return {}; }
  function saveProgress() { /* no-op */ }
  let progress = {};
  function isDone() { return false; }
  function markDone() { return false; }

  // -------------------------------------------------- last-visited (resume) --
  const LAST_KEY = "zuhyoSahoLast:v1";
  function saveLast() { /* no-op: 履歴を残さない */ }
  function loadLast() { return null; }
  ...
  function viewHome() {
    ...
    const cards = chs.map((ch) => {
      ...
      return `<a class="chapter-card" href="#/${ch.id}" data-no="${String(ch.number).padStart(2, "0")}" style="--accent-color:${color};--accent-soft-color:${soft}">
        ...
        <h3>${ch.title}</h3>
        <p>${ch.description || ""}</p>
        <div class="meta-row"><span>${liveLabel}</span><span>タップして開く</span></div>
      </a>`;
    }).join("");
    ...
    const resumeHtml = resume ? `
      <a class="resume-card" href="#/${resume.ch.id}/${resume.t.id}"
        ...
        <span class="resume-tag">はじめて学ぶ方はここから</span>
    ...
    mount(`
      <div class="wrap">
        <section class="hero">
          ...
          <h1>論文図表を<br/><span class="accent">一から丁寧に</span>学ぶ</h1>
          <p class="lead">統計の基礎から臨床試験の解釈まで、章立てに沿って体系的に学習できる教材です。書籍の構成に準拠し、各手法の原理を解説するとともに、図表は実際に操作しながら確認できる形で再現しています。</p>
          <div class="hero-stats">
            ...
            <div class="stat"><b>${chs.length}</b><span>書籍全体の章数</span></div>
          </div>
        </section>
        ${resumeHtml}
  ...
  /* トピック冒頭の代表画像。img/<chId>/<topicId>.webp を規約で参照する（対応表は不要）。
     画像が無い項目でも崩れないよう、onerror で figure ごと消す。 */
  function renderTopicImageHtml(ch, topic) {
    const src = "img/" + ch.id + "/" + topic.id + ".webp";
    const alt = topic.title + "（" + (topic.en || ch.title) + "）の概念図";
    return `
      <figure class="topic-img" style="--accent-color:${ch.color}">
        <img src="${src}" alt="${alt}" width="1400" height="788"
             loading="lazy" decoding="async"
             onerror="this.closest('.topic-img').remove()" />
      </figure>`;
  }
